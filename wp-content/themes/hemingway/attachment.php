			...
			<div class="entrytext">
				...
				<p class="postmetadata alt">
					<small>
						...
						<?php _e("on",TEMPLATE_DOMAIN); ?> <?php the_time('l, F jS, Y') ?> <?php _e("at",TEMPLATE_DOMAIN); ?> <?php the_time() ?>
						<?php _e("and is filed under",TEMPLATE_DOMAIN); ?> <?php the_category(', ') ?><?php if (is_callable('the_tags')) the_tags(' with tags ', ', '); ?>.
						<?php _e("You can follow any responses to this entry through the <?php comments_rss_link('RSS 2.0'); ?> feed.",TEMPLATE_DOMAIN); ?>
						
						<?php if (('open' == $post-> comment_status) && ('open' == $post->ping_status)) {
							// Both Comments and Pings are open ?>
							<?php _e("You can",TEMPLATE_DOMAIN); ?> <a href="#respond"><?php _e("leave a response",TEMPLATE_DOMAIN); ?></a>, <?php _e("or",TEMPLATE_DOMAIN); ?> <a href="<?php trackback_url(true); ?>" rel="<?php _e("trackback",TEMPLATE_DOMAIN); ?>"><?php _e("trackback",TEMPLATE_DOMAIN); ?></a> <?php _e("from your own site.",TEMPLATE_DOMAIN); ?>
