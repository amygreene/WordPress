...
<?php if ($comments) : ?>
  ...
  <div class="post">
	...
	<?php foreach ($comments as $comment) : ?>
		...
		<li id="comment-<?php comment_ID() ?>" <?php if ($comment->comment_approved == '0') : ?>class="moderation"<?php endif; ?>>
		
		  <div class="commentText">
			<?php if ($comment->comment_approved == '0') : ?>
			<h3><em>Your comment is awaiting moderation.</em></h3>
			<?php endif; ?>
			<?php comment_text() ?>
			<span>&nbsp;</span>
			</div>
			...
			<p class="commentmetadata"><?php comment_date('F jS, Y') ?> at <?php comment_time() ?><a href="#comment-<?php comment_ID() ?>" title="" class="commentLink">permalink</a> <?php edit_comment_link('edit','&nbsp;&nbsp;',''); ?></p>
			</div>
...
<?php if ( $user_ID ) : ?>
<p>Logged in as <strong><a href="<?php echo get_option('siteurl'); ?>/wp-admin/profile.php"><?php echo $user_identity; ?></a></strong>. <a href="<?php echo get_option('siteurl'); ?>/wp-login.php?action=logout" title="Log out of this account">Log out &raquo;</a></p>
<?php else : ?>

<p><input type="text" name="author" id="author" value="<?php echo $comment_author; ?>" size="22" tabindex="1" class="field" />
<label for="author">Name: <?php if ($req) echo '<span class="req">*</span>'; ?></label></p>
<p><input type="text" name="email" id="email" value="<?php echo $comment_author_email; ?>" size="22" tabindex="2" class="field" />
<label for="email">Email: <?php if ($req) echo '<span class="req">*</span>'; ?></label></p>
