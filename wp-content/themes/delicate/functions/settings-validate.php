        ...
        switch ($option['filter']) {
           ...
           case 'num':
              ...
              if(is_numeric($input[$option['name']]) == FALSE) {  
                 add_settings_error($option['name'], 'nattywp_txt_numeric_error', __('Expecting a Numeric value! Please fix.','delicate'), 'error');  
              }
           break;
      ...
      if ($image['size']) { // if a files was upload
        if (preg_match('/(jpg|jpeg|png|gif|x-icon)$/', $image['type'])) {
            ...
            if(!empty($uploaded_file['error'])) {
                echo json_encode(array('error'=>'-1', 'message'=> __('Upload Error: ', 'delicate').$uploaded_file['error'])); 
            }	else {
                // no errors, do update                
           ...
           // Not an image.
           echo json_encode(array('error'=>'-1', 'message'=> __('Wrong image type. Upload only .jpeg, .png or .gif images.', 'delicate')));
           die();
         }
       }
    }
