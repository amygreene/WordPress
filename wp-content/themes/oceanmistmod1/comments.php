...
<?php if ($comments) : ?>
		<div class="title">
		  <h2>Responses  - <?php if (function_exists('comment_form_text_output')){ comment_form_text_output(); } ?></h2>
	...
	<?php foreach ($comments as $comment) : ?>
		...
		<div class="archive" id="comment-<?php comment_ID() ?>">			
			    ...
			    <div class="entry">
				      <?php comment_text() ?>
				  <ul class="buttons">
				    <li><?php edit_comment_link('Edit Comment','',''); ?></li>
				  </ul>
		        </div>
			    <div class="postinfo">
			      <p><?php if (function_exists('avatar_display_comments')){ avatar_display_comments(get_comment_author_email(),'48',''); } ?>&nbsp;&nbsp;
By: <strong><?php comment_author_link() ?></strong> <?php _e('on');?> <?php comment_date() ?> <br /> at <?php comment_time() ?></p>
...
<?php if ('open' == $post->comment_status) : ?>
		...
		<div class="title">
		  <h2>Leave a response - <?php if (function_exists('comment_form_text_output')){ comment_form_text_output(); } ?></h2>
		</div>
		<div class="post">
  ...
  <div class="postinfo">
    ...
    <?php if ( $user_ID ) : ?>
    <p><?php _e('Logged in as');?> <a href="<?php echo get_option('siteurl'); ?>/wp-admin/profile.php"><?php echo $user_identity; ?></a></p>
    <ul class="buttons">
