...
<div class="post-content">
  <p>
    ...
    <span class="trackbackr"><a href="<?php trackback_url(display); ?>">Trackback <acronym title="<?php _e('Uniform Resource Identifier');?>">URI</acronym></a></span> |
    <?php } ?>
...
<h3 id="comments">
  <?php comments_number(__('No Responses',TEMPLATE_DOMAIN), __('One Response',TEMPLATE_DOMAIN), __('% Responses',TEMPLATE_DOMAIN ));?> <?php _e('to',TEMPLATE_DOMAIN);?>  &#8220;
  <?php the_title(); ?>
  &#8221;</h3>
...
<?php cancel_comment_reply_link(); ?>
  ...
  <p>
    ...
    <label for="author"><small><?php _e('Name',TEMPLATE_DOMAIN);?> <?php if ($req) echo __('(required)',TEMPLATE_DOMAIN); ?>
    </small></label>
  </p>
  <p>
    <input type="text" name="email" id="email" value="<?php echo $comment_author_email; ?>" size="22" tabindex="2" />
    <label for="email"><small><?php _e("Mail (will not be published)",TEMPLATE_DOMAIN); ?>
    <?php if ($req) echo __('(required)',TEMPLATE_DOMAIN); ?>
    </small></label>
  </p>
  <p>
