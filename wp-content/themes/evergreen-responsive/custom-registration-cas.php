<?php
/*
Template Name: Custom Registration CAS
*/
...
function show_user_form($user_name = '', $user_email = '', $errors = '') {
	// User name
	echo '<label for="user_name">' . __('Evergreen&nbspUsername:') . '</label>';
	if ( $errmsg = $errors->get_error_message('user_name') ) {
		echo '<p class="error">'.$errmsg.'</p>';
	}
	echo '<input class="locked" name="user_name" type="text" id="user_name" value="'. esc_attr($user_name) .'" maxlength="60" readonly/><br />';
	_e( '(This is the same username used to login to my.evergreen.edu.)' );
	?>

	<label for="user_email"><?php _e( 'Evergreen&nbspEmail&nbsp;Address:' ) ?></label>
	<?php if ( $errmsg = $errors->get_error_message('user_email') ) { ?>
		<p class="error"><?php echo $errmsg ?></p>
	<?php } ?>
	<input class="locked" name="user_email" type="text" id="user_email" value="<?php  echo esc_attr($user_email) ?>" maxlength="200" readonly/><br /><?php _e('(Your username@evergreen.edu)') ?>
	<?php
...
function signup_user($user_name = '', $user_email = '', $errors = '') {
	...
	$user_email = $user_name . '@evergreen.edu';
	$errors = $filtered_results['errors'];

	?>

	<h2>Welcome <?php echo $user_name?>!</h2>
	<p><?php printf( __( 'New to %s? Use your Evergreen username and email to setup an account in seconds!<br />' ), $current_site->site_name ) ?>
	<em>Not your account? <a href="<?php echo wp_logout_url(); ?>" title="Logout">Logout</a></em></p>
	<form id="setupform" method="post" action="wp-signup.php">
