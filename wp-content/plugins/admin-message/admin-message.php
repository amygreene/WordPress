...
function admin_message_plug_pages() {
	...
	if ( version_compare($wp_version, '3.0.9', '>') ) {
		if ( is_network_admin() ) {
			add_submenu_page($admin_message_settings_page, __('Admin Message', 'admin_message'), __('Admin Message', 'admin_message'), 10, 'admin-message', 'admin_message_page_output');
		}
	} else {
		if ( is_site_admin() ) {
			add_submenu_page($admin_message_settings_page, __('Admin Message', 'admin_message'), __('Admin Message', 'admin_message'), 10, 'admin-message', 'admin_message_page_output');
...
function admin_message_page_output() {
	...
	switch( $_GET[ 'action' ] ) {
		...
		default:
            ...
            <p class="submit">
            <input type="submit" name="Submit" value="<?php _e('Save Changes', 'admin_message') ?>" />
			<input type="submit" name="Reset" value="<?php _e('Reset', 'admin_message') ?>" />
            </p>
            </form>
			<?php
		break;
...
//commented out to remove annoying wpmudev dashboard message
