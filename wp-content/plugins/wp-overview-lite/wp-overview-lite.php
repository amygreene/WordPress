 ...
 *
 * [WP Overview (lite)](http://wordpress.org/extend/plugins/wp-overview-lite/) WordPress PlugIn
 *
 ...
 *
 * Copyright © 2009-2011 [sLa](http://wordpress.org/extend/plugins/profile/slangji) (slangji[at]gmail[dot]com)
 * 
 * Thanks to [RavanH](http://profiles.wordpress.org/users/RavanH) for this [security](http://wordpress.org/support/topic/plugin-wp-overview-lite-dashboard-footer-memory-usage-request-dashboard-widget-not-for-subscribers) fix.
 */
/**
 ...
 * @license GPLv2
 *
 * Show Dashboard Overview and Memory Load Usage on Footer.
 */
if(!function_exists('add_action')){header('HTTP/1.0 403 Forbidden');header('HTTP/1.1 403 Forbidden');exit();}?><?php
function wpo_footer_log(){echo"\n<!--Plugin WP Overview (lite) 2011.0723.2011 Active-->";}add_action('wp_head','wpo_footer_log');add_action('wp_footer','wpo_footer_log')?><?php
if(is_admin()){class wp_overview_lite{var$memory=false;function wpo(){return$this->__construct();}function __construct(){add_action('init',array(&$this,'wpo_limit'));add_action('wp_dashboard_setup',array(&$this,'wpo_dashboard'));add_action('wp_network_dashboard_setup',array(&$this,'wpo_network_dashboard'));add_filter('admin_footer_text',array(&$this,'wpo_footer'));$this->memory=array();}function wpo_limit(){$this->memory['wpo-limit']=(int)ini_get('memory_limit');}function wpo_load(){$this->memory['wpo-load']=function_exists('memory_get_usage')?round(memory_get_usage()/1024/1024,2):0;}function wpo_consumption(){$this->memory['wpo-consumption']=round($this->memory['wpo-load']/$this->memory['wpo-limit']*100,0);}function wpo_output(){$this->wpo_load();$this->wpo_consumption();$this->memory['wpo-load']=empty($this->memory['wpo-load'])?__('0'):$this->memory['wpo-load'].__('M')?><?php
global$wpdb,$wp_version,$wpmu_version;$cur_locale=get_locale();$mysql_status=array();$mysql_vars=array();foreach($wpdb->get_results('SHOW GLOBAL STATUS')as$result){$mysql_status[$result->Variable_name]=$result->Value;}foreach($wpdb->get_results('SHOW GLOBAL VARIABLES')as$result){$mysql_vars[$result->Variable_name]=$result->Value;}$uptime_days=$mysql_status['Uptime']/86400;$uptime_hours=($uptime_days-(int)$uptime_days)*24;$uptime_minutes=($uptime_hours-(int)$uptime_hours)*60;$uptime_seconds=($uptime_minutes-(int)$uptime_minutes)*60;$uptime_string=(int)$uptime_days.' days, '.(int)$uptime_hours.' hours, '.(int)$uptime_minutes.' minutes, '.(int)$uptime_seconds.' seconds'?>
...
<em><strong>Legend</strong> 0=disabled 1=enabled</em><?php
}function wpo_dashboard(){if(!current_user_can('unfiltered_html'))return;wp_add_dashboard_widget('dashboard_overview','Overview',array(&$this,'wpo_output'));}function wpo_network_dashboard(){if(!current_user_can('unfiltered_html'))return;wp_add_dashboard_widget('network_dashboard_overview','Overview',array(&$this,'wpo_output'));}function wpo_footer($content){$this->wpo_load();$content.=' ~ load '.$this->memory['wpo-load'].' of '.$this->memory['wpo-limit'].'M';return$content;}}add_action('plugins_loaded',create_function('','$memory=new wp_overview_lite();'));}?><?php
if(is_admin()){function wpo_fs_info($filesize){$bytes=array('B','K','M','G','T');if($filesize<1024)$filesize=1;for($i=0;$filesize>1024;$i++)$filesize/=1024;$wpo_fs_info['size']=round($filesize,3);$wpo_fs_info['type']=$bytes[$i];return$wpo_fs_info;}function wpo_db_size(){$rows=mysql_query("SHOW table STATUS");$dbsize=0;while($row=mysql_fetch_array($rows)){$dbsize+=$row['Data_length']+$row['Index_length'];}$dbsize=wpo_fs_info($dbsize);echo"db {$dbsize['size']}{$dbsize['type']}";}add_filter('admin_footer_text','wpo_db_size');}?>